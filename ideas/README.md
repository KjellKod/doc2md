...
| `if-needed` | `mac-open-with-closed-window` | Finder open when the single main window was closed but the app is still running: re-create the window so the buffered URL is delivered. Narrow edge from PR #166 review. See [`mac-open-with-closed-window.md`](mac-open-with-closed-window.md). |
