...
2. **Phase 5d production Sparkle update UX**
   - Status: planned in the [Mac Sparkle Update Roadmap](../docs/implementation/mac-sparkle-update-roadmap.md).
   - Scope: production appcast URL, startup update-check behavior, skip/cancel/download/install-restart validation, and optional Cloudflare `updates.doc2md.dev` setup.
   - This can be one PR if Cloudflare setup is complete first; otherwise split GitHub-backed updater behavior from the later Cloudflare feed migration.
...
- Add a lightweight licensing model for the Mac app only (amended 2026-07-07):
  ...
  - One-time online activation at license entry against Polar's customer-portal API; silent revalidation inside the 14-day window around `expires_at` (7 before, 7 after) plus throttled retries while cached entitlement is expired without a definitive answer; no merchant secrets in the app; every licensing call non-blocking for document operations. The dormant Ed25519 verifier stays as the v2 contingency.
