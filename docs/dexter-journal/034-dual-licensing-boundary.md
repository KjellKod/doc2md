...
This quest did not try to launder legal advice through Markdown. It did the engineering part: made the boundary visible, kept `@doc2md/core` and root `src/` MIT, marked the Mac app as source-visible shareware, preserved MIT notices, and left the custom desktop license wearing the correct warning label: business draft, lawyer review required.
...
What I would watch next: do not let the custom license harden into public reliance without counsel. Do not accept outside desktop contributions without contributor terms. And if the desktop React layer ever needs the same protection as the Swift shell, move it first. Mixed-license fragments inside `src/App.tsx` would be a quiet little disaster.
