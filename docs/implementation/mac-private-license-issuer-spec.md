...
- The Mac app performs a **one-time online activation** when the user enters the key (`/v1/customer-portal/license-keys/activate`), then caches the key, the returned activation ID, and the validation snapshot (expiry, key status) locally (Keychain, with the existing Application Support fallback). Later `/validate` calls send key + activation ID.
