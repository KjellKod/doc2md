...
- The Mac app performs a **one-time online activation** when the user enters the key (`/v1/customer-portal/license-keys/activate`). The key and the returned activation ID are stored in the non-syncing Keychain only; the Application Support fallback holds only non-secret validation metadata (key status, expiry, last-validated timestamp), never the raw key, because a Polar key is a reusable bearer credential. Later `/validate` calls send key + activation ID. If the Keychain is unavailable, the app asks the user to re-enter the key; because the lost activation ID may still occupy an activation slot, an `/activate` failure on the activation limit must surface a recovery path (self-service deactivation in Polar's customer portal, or support-assisted slot reset through `support@doc2md.dev`), and the product's activation limit is configured with headroom so one lost slot never locks out a paid user. Removing or replacing a license calls the public `/deactivate` endpoint (best-effort, non-blocking) before clearing local credentials, so ordinary license changes do not leak activation slots.
- **Revalidation is quiet while licensed**: silent background attempts start 7 days before `expires_at` and continue through the 14-day window (7 before, 7 after). A renewed subscription picks up a new expiry invisibly. Once the cached entitlement is expired while the key status is still `granted`, throttled attempts (once per launch or per day) continue until validation succeeds or returns `revoked`/`disabled`; an expired-unchanged `granted` answer does not stop the retries, so a renewal on the same key, even a late one, is always detected and the customer recovers to licensed automatically. The cached `expires_at` is authoritative: a successful validation affects entitlement only through the snapshot it writes, and never extends a license past its cached expiry. There is no phone-home while validly licensed and far from expiry. Validation uses `/v1/customer-portal/license-keys/validate`.
- These customer-portal endpoints are designed for client-side calls keyed by the license key itself. Requests also carry Polar's public `organization_id`, and `/activate` includes a privacy-safe device label: the generic hardware model plus a short persistent non-secret installation suffix so same-model Macs stay distinguishable in the customer portal (never a personalized hostname or username). The `organization_id` is a public identifier that may ship in app configuration in this repo; it is not a merchant credential, and this spec's ban on merchant account identifiers refers to credentials and private account data, not this public ID. The app must embed **no merchant secret, organization token, or API credential**. The integration quest must verify these properties against current Polar documentation before shipping.
