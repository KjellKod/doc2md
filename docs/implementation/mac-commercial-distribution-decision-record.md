...
- **Interim issuer:** Polar's license-key API. One-time online activation at license entry; silent revalidation inside a 14-day window around `expires_at` (7 days before through 7 days after), and throttled retries while the cached entitlement is expired without a definitive answer, so a renewed customer always recovers to licensed; cached license state otherwise, with no phone-home while validly licensed and far from expiry. All licensing network calls are non-blocking for document operations. The custom Ed25519 issuer is deferred; see the [issuer spec](mac-private-license-issuer-spec.md).
