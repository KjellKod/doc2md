...
- Request contract: both `/activate` and `/validate` carry Polar's **public `organization_id`**, shipped as build-time app configuration. It is a public identifier, not a merchant credential, and may live in this source-visible repo. `/activate` additionally sends a **privacy-safe device label** shown in Polar's customer portal for self-service deactivation: the generic hardware model plus a short persistent non-secret installation suffix (e.g. "MacBook Pro (14-inch) · 7Q2F"), never the personalized hostname or username. The suffix is generated at first activation and stored with the non-secret metadata in Application Support, so it survives Keychain loss, which is exactly when the user needs to tell two same-model Macs apart. Mocked tests assert both required fields are present and that two same-model installations produce distinguishable labels.
...
- Revalidation calls (`/validate`) send key + activation ID and fire silently when the app is open and online: inside the 14-day window, and **also whenever the cached entitlement is expired while the key status is still `granted`** (throttled, once per launch or per day). Only a `revoked`/`disabled` answer or the user removing the license stops the retries; an expired-unchanged `granted` response ends `grace` (Phase 1) but does **not** stop the retry loop, because a later renewal on the same key (delayed payment retry, resumed subscription) must still be detected. The state stays `expiredReminder` until a validation succeeds; success refreshes the snapshot and the next state read returns `licensed`. Without this, a renewed customer who does not open the app during the window would stay expired forever.
- Removing or replacing a license first calls `POST /v1/customer-portal/license-keys/deactivate` (key, organization ID, activation ID) to free the activation slot, then clears the local credentials. Deactivation is best-effort and non-blocking: when it fails (offline, API error), the removal still completes locally and the app notes that the slot may remain occupied, pointing at the Polar portal recovery path. Without this, ordinary license changes leak slots until the limit locks the customer out.
...
- Activation, revalidation, and failure paths covered by tests against a mocked API; no test talks to live Polar. Includes the late-renewal case: an expired `granted` response followed by a later validation returning a new `expires_at` must transition back to `licensed`.
...
- License removal and replacement are tested online and offline: online frees the activation slot via `/deactivate` before clearing credentials; offline still completes the local removal, never blocks, and surfaces the occupied-slot note.
