...
- Request contract: both `/activate` and `/validate` carry Polar's **public `organization_id`**, shipped as build-time app configuration. It is a public identifier, not a merchant credential, and may live in this source-visible repo. `/activate` additionally sends a **privacy-safe device label** shown in Polar's customer portal for self-service deactivation: the generic hardware model (e.g. "MacBook Pro (14-inch)"), never the personalized hostname or username. Mocked tests assert both required fields are present.
