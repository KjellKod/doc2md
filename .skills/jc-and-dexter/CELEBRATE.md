...
- Tombstone block-letter ASCII art inside a triple-backtick fenced code block (see `celebrate/SKILL.md` for rendering rules)
...
JC renders tombstone block-letter art for the quest name inside a triple-backtick fenced code block (gothic/memorial aesthetic, not party). See `celebrate/SKILL.md` "Requiem Rendering" section for block-letter examples and emoji palette.
