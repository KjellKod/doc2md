**Status: Complete | Owner: maintainers | Date: 2026-03-29 | Completed: 2026-03-30**
---

## Overview

**Problem: The current doc2md working tree is in decent shape, but several quality debts remain**
concentrated in three areas: indirect-only coverage for shared helpers, shallow top-level UI
validation, and a growing orchestration hook that is still carrying too many responsibilities.
**Impact: This work should improve confidence in future changes without changing product**
behavior. The main outcome is safer refactoring, stronger regression detection, and clearer
boundaries in the most change-prone parts of the codebase.
**Scope In:**
...
**Non-Negotiable Constraint: In all cases, we drive the work so existing functionality does not**
break. This means characterization tests first, refactor second, and no intentional behavior
...
- shared helper coverage in delimited.ts, readBinary.ts, readText.ts,
**richText.ts, office.ts**
- shallow top-level test coverage in App.test.tsx
- refactor pressure in useFileConversion.ts

## Acceptance Criteria

1. Existing user-facing behavior remains unchanged unless a change is explicitly
documented and approved.

2. Before any structural refactor, characterization tests exist for the behavior being
protected.
...
1. A stronger top-level UI flow test exists for the core user journey:
upload/select/preview/download readiness.
2. useFileConversion.ts is simplified or decomposed in a behavior-preserving way,
with direct tests covering its protected behavior.
3. Final validation passes using the repo's real test command(s), and any test-count drift is
explained in the quest artifacts.
4. The implementation stays incremental: small slices, test-backed, reviewable, and
bounded.
...
**Deliverable: New or expanded tests that define current expected behavior before cleanup work**
begins.

### Phase 2: Helper-Layer Coverage

**Goal: Add direct tests around shared parsing and file-I/O behavior that multiple converters**
depend on.
**Target files:**
- src/converters/delimited.ts
- src/converters/readBinary.ts
...
**Constraint: Prefer testing-library style interaction tests over brittle implementation-detail**
assertions.

### Phase 4: Behavior-Preserving Hook Refactor

**Goal: Reduce the maintenance pressure in useFileConversion.ts without changing its**
external behavior.
**Primary target: src/hooks/useFileConversion.ts**
**Refactor direction:**
...
**Constraint: Skip any cleanup item that turns into a broader architectural change.**
...
- convertHtmlFragmentToMarkdown() / Google Docs nesting helpers — shared
HTML-to-markdown behavior
...
- Files: src/converters/*.test.ts and new direct helper tests as added
...
**System: Third-party document parsing libraries (mammoth, pdfjs-dist,**
**read-excel-file, turndown)**
...
- ✅ Overview is bounded and explicit
- ✅ Acceptance criteria are observable and testable
- ✅ Validation includes concrete commands
- ✅ Integration touchpoints include "could break" and validation
- ✅ Risks are documented with mitigations
- ✅ Scope is cleanup/refactor/test-quality only
- ✅ Existing functionality preservation is explicit