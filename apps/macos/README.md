...
- `LICENSES/LicenseRef-doc2md-Desktop.txt` as `doc2md.app/Contents/Resources/LicenseRef-doc2md-Desktop.txt` for compliance. The desktop product license is bundled-only and intentionally has no in-app affordance.
...
- `Licenses` opens a native, read-only window titled `Third-Party Licenses` whose body is the bundled `apps/macos/THIRD_PARTY_NOTICES.md` content with the leading top-heading line stripped so the synthetic `Third-Party Licenses` title is the only top heading. The body is selectable and scrollable.
...
Help-menu third-party items have been removed: there is no longer an `Acknowledgments…` item or a third-party `License…` item under Help. The third-party notices source of truth remains the bundled `apps/macos/THIRD_PARTY_NOTICES.md`, surfaced through the About panel's `Licenses` button.
